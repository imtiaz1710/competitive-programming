   ...
   // fin;
    ...
    for (ll i = 0; i < m; i++)
    {
        ...
        if(mp[tmp]!=1){
            ll indx1=mp[tmp];
            ll indx2=mp[tmp]-1;
            swap(b[indx1],b[indx2]);
            mp[b[indx1]]=indx1;
            mp[b[indx2]]=indx2;
            highest[b[indx1]]=max(highest[b[indx1]],indx1);
            highest[b[indx2]]=max(highest[b[indx2]],indx2);
            lowest[b[indx1]]=min(lowest[b[indx1]],indx1);
            lowest[b[indx2]]=min(lowest[b[indx2]],indx2);
        }
    }
    for (ll i = 1; i <=n; i++)
    {
        OUTT(lowest[i],highest[i]);
    }
