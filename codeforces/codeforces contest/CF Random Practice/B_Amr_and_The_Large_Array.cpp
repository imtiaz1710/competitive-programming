...
int main(){
    ...
    for (ll i = 0; i < n; i++)
    {
        ...
        IN(in);
        mp[in]++;
        if(!left[in])left[in]=i+1;
        right[in]=i+1;
    }
    for(auto tmp:mp) a.PB({tmp.SS,tmp.FF});
    RSORT(a);
    ll mx=a[0].FF;
    PLL idx;
    ll ans=MOD;
    for (ll i = 0; i < a.size() and a[i].FF==mx; i++)
    {
        if( ans> (right[a[i].SS]-left[a[i].SS]+1)  )
        {
            ans=right[a[i].SS]-left[a[i].SS]+1;
            idx={left[a[i].SS],right[a[i].SS]};
        }
    }
