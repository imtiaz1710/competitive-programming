...
ll a[1000000];
map<PLL,ll> dp; 
ll sol(ll id,ll prev){
    if(id==n){
        return 0;
    }
    if(dp[{id,prev}]!=0) return dp[{id,prev}];
    ll nibo=0;
    ll nibona=0;
    if (prev == -1 or id - prev == a[id] - a[prev])
    {
        nibo=a[id]+sol(id+1,id);
    }
    nibona=sol(id+1,prev);
    return dp[{id,prev}]=max(nibo,nibona);
}
int main(){
   // fin;
    IN(n);
